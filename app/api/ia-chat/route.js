...
const TOOLS=[
  ...
  {type:'function',function:{name:'gerar_imagem',description:'Gera imagem com IA a partir de uma descriÃ§Ã£o em texto - retorna URL da imagem',parameters:{type:'object',properties:{descricao:{type:'string'},largura:{type:'number'},altura:{type:'number'}},required:['descricao']}}},
  {type:'function',function:{name:'analisar_imagem',description:'Analisa e descreve o conteÃºdo de qualquer imagem via URL',parameters:{type:'object',properties:{url:{type:'string'},pergunta:{type:'string'}},required:['url']}}},
...
async function runTool(name,args){
  try{
    ...
    if(name==='github_read_file'){
      ...
      if(!r.ok)return`âŒ NÃ£o encontrado: ${args.path}`;
      const d=await r.json();
      return`ğŸ“„ **${args.path}** (${d.size}b)\n\`\`\`\n${b64d(d.content).substring(0,8000)}\n\`\`\``;
    }

    if(name==='github_list_dir'){
      const r=await ghReq(args.path||'',{},repo);
      if(!r.ok)return`âŒ Dir nÃ£o encontrado`;
      const items=await r.json();
      return`ğŸ“ ${args.path||'/'} (${items.length} itens)\n${items.map(i=>`${i.type==='dir'?'ğŸ“':'ğŸ“„'} ${i.name} ${i.size?`(${i.size}b)`:''}`).join('\n')}`;
    }

    if(name==='github_write_file'){
      const sha=await ghCommit(repo,args.path,args.content,args.message);
      return`âœ… **Commitado:** \`${args.path}\` â†’ SHA:${sha}\nğŸš€ Deploy Vercel disparado automaticamente`;
    }
    ...
    if(name==='github_create_repo'){
      ...
      if(r.status===422)return`âš ï¸ Repo "${args.name}" jÃ¡ existe`;
      const d=await r.json();return`âœ… Repo criado: ${d.html_url}`;
    }
    ...
    if(name==='supabase_select'){
      ...
      const d=await r.json();
      return`ğŸ“Š **${args.table}** (${d.length} registros)\n\`\`\`json\n${JSON.stringify(d,null,2).substring(0,4000)}\n\`\`\``;
    }
    ...
    if(name==='supabase_sql'){
      ...
      const d=await r.json();
      return d.error?`âŒ SQL erro: ${d.error}`:`âœ… SQL OK:\n\`\`\`json\n${JSON.stringify(d.data,null,2).substring(0,3000)}\n\`\`\``;
    }
    ...
    if(name==='pesquisar_web'){
      ...
      if(!results.length){
        ...
        if(d.RelatedTopics)d.RelatedTopics.slice(0,3).forEach(t=>{if(t.Text)results.push(`â€¢ ${t.Text}`);});
      }
      return results.length?`ğŸ” **Resultados para "${args.query}":**\n\n${results.join('\n\n---\n\n')}`:`âŒ Sem resultados para: ${args.query}`;
    }
    ...
    if(name==='executar_codigo'){
      ...
      if(!r.ok)return`âŒ Piston API erro: ${r.status}`;
      const d=await r.json();
    ...
    if(name==='gerar_imagem'){
      ...
      return`ğŸ¨ **Imagem em geraÃ§Ã£o...**\n\nURL: ${url}\n\n_Copie a URL e abra no browser para ver a imagem_`;
    }

    if(name==='analisar_imagem'){
      if(!GEK)return`âŒ Gemini API key nÃ£o configurada`;
    ...
    if(name==='memoria_salvar'){
      ...
      const d=await r.json();
      return d.error?`âŒ Erro ao salvar: ${d.error}`:`ğŸ§  **MemÃ³ria salva:** "${args.chave}" â†’ guardado para conversas futuras`;
    }
    ...
    if(name==='memoria_carregar'){
      const r=await fetch(`${SBU}/rest/v1/ia_cache?cache_key=eq.mem_${args.chave}&select=value,expires_at`,
        {headers:{apikey:SBK,Authorization:`Bearer ${SBK}`}});
      const d=await r.json();
      if(!d.length)return`âŒ MemÃ³ria "${args.chave}" nÃ£o encontrada`;
      return`ğŸ§  **MemÃ³ria carregada:** "${args.chave}"\n\n${d[0].value}`;
    }
  ...
  }catch(e){return`âŒ Erro em ${name}: ${e.message}`;}
}
...
const SYS=`VocÃª Ã© Daniela â€” IA ULTRA autÃ´noma, igual ao Claude mas 100% grÃ¡tis.
VocÃª TEM as seguintes capacidades REAIS:
