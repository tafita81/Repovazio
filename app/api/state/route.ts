...
export async function GET() {
  ...
  if (regs && regs.length > 0) {
    const inicio = new Date(regs[0].created_at);
    diaAtual = Math.floor((Date.now() - inicio.getTime()) / (1000 * 60 * 60 * 24)) + 1;
  }
  const membrosArr = membros as any[];
  const membrosCount = Number(membrosArr?.[0]?.count || 0);
  return Response.json({ plano: plano || [], membros_wa: membrosCount, dia_atual: diaAtual, hora_sp: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }) });
}