...
export default function Chat(){
  const[msgs,setMsgs]=useState([]);const[voiceOn,setVoiceOn]=useState(false);const[voiceText,setVoiceText]=useState('');const[voiceBusy,setVoiceBusy]=useState(false);const[voiceListening,setVoiceListening]=useState(false);const voiceOnRef=useRef(false);const recRef=useRef(null);const vmsgRef=useRef([]);const bestVoiceRef=useRef(null);const audioRef=useRef(null);async function vPlayNeural(text){try{const res=await fetch('/api/ia-chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'tts',text:text})});const d=await res.json();if(d&&d.audio_base64){if(!audioRef.current)audioRef.current=new Audio();audioRef.current.onended=function(){if(voiceOnRef.current)setTimeout(vRec,200);};audioRef.current.onerror=function(){vSpeak(text);};audioRef.current.src='data:audio/wav;base64,'+d.audio_base64;const _p=audioRef.current.play();if(_p&&_p.catch)_p.catch(function(){vSpeak(text);});return;}}catch(e){}vSpeak(text);}function vSpeak(t){try{if(!bestVoiceRef.current){const _vs=speechSynthesis.getVoices();if(_vs.length){const _pt=_vs.filter(v=>/^pt/i.test(v.lang));bestVoiceRef.current=_pt.find(v=>/google/i.test(v.name))||_pt.find(v=>/siri|premium|enhanced|aprimorada|neural|francisca|joana/i.test(v.name.toLowerCase()))||_pt.find(v=>/luciana/i.test(v.name))||_pt[0]||null;}else{speechSynthesis.onvoiceschanged=function(){bestVoiceRef.current=null;vSpeak(t);};return;}}speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(t);u.lang='pt-BR';u.rate=0.94;u.pitch=1.0;if(bestVoiceRef.current)u.voice=bestVoiceRef.current;u.onend=function(){if(voiceOnRef.current)setTimeout(vRec,200);};speechSynthesis.speak(u);}catch(e){if(voiceOnRef.current)setTimeout(vRec,200);}}function vRec(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){alert('Seu navegador nao suporta reconhecimento de voz. Atualize o Safari ou Chrome.');return;}try{if(recRef.current)recRef.current.abort();}catch(e){}const r=new SR();r.lang='pt-BR';r.interimResults=true;r.continuous=false;r.maxAlternatives=1;recRef.current=r;setVoiceListening(true);setVoiceText('');r.onresult=function(e){let t='';for(let i=0;i<e.results.length;i++)t+=e.results[i][0].transcript;setVoiceText(t);if(e.results[e.results.length-1].isFinal){setVoiceListening(false);try{r.stop();}catch(x){}vAsk(t);}};r.onerror=function(){setVoiceListening(false);};r.onend=function(){setVoiceListening(false);};try{r.start();}catch(e){}}async function vAsk(text){if(!text||!text.trim())return;setVoiceBusy(true);const base=vmsgRef.current.length?vmsgRef.current:msgs;const nm=[...base,{role:'user',content:text}];vmsgRef.current=nm;setMsgs(nm);try{const res=await fetch('/api/ia-chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:nm,session_id:sessionId,voice:true})});const d=await res.json().catch(function(){return{reply:'Nao entendi, pode repetir?'};});const reply=(d&&d.reply)||'Nao entendi, pode repetir?';const fm=[...nm,{role:'assistant',content:reply}];vmsgRef.current=fm;setMsgs(fm);try{saveSession(fm);}catch(x){}setVoiceText('');setVoiceBusy(false);vPlayNeural(reply);}catch(e){setVoiceBusy(false);vSpeak('Tive um problema de conexao.');}}function vToggle(){if(voiceOnRef.current){voiceOnRef.current=false;setVoiceOn(false);setVoiceListening(false);try{if(recRef.current)recRef.current.abort();}catch(e){}try{speechSynthesis.cancel();}catch(e){}return;}voiceOnRef.current=true;setVoiceOn(true);vmsgRef.current=msgs.slice();try{const w=new SpeechSynthesisUtterance(' ');w.lang='pt-BR';speechSynthesis.speak(w);}catch(e){}try{if(!audioRef.current)audioRef.current=new Audio();audioRef.current.src='data:audio/wav;base64,UklGRigAAABXQVZFZm10IBAAAAABAAEAgD4AAAB9AAACABAAZGF0YQQAAAAAAAAA';audioRef.current.play().catch(function(){});}catch(e){}vRec();}
  ...
  return(
    ...
    <div className="app">
      ...
      <main className="main">
        ...
        {/* INPUT */}
        <div className="ia">
          {voiceOn&&(<div onClick={function(e){if(e.currentTarget===e.target)vToggle();}} style={{position:'fixed',inset:0,zIndex:9999,background:'rgba(6,6,15,0.96)',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)'}}><div style={{textAlign:'center',maxWidth:'90%',padding:24}}><div style={{fontSize:84,marginBottom:18,filter:voiceListening?'drop-shadow(0 0 28px #7C3AED)':'none',transition:'all .25s'}}>{voiceBusy?'💭':voiceListening?'🎙️':'🎤'}</div><div style={{color:'#fff',fontSize:21,marginBottom:10,fontWeight:600}}>{voiceBusy?'Pensando...':voiceListening?'Ouvindo... pode falar':'Toque em Falar e fale'}</div><div style={{color:'#A78BFA',fontSize:16,minHeight:26,marginBottom:26,fontStyle:'italic'}}>{voiceText}</div><div style={{display:'flex',gap:12,justifyContent:'center'}}><button onClick={vRec} disabled={voiceBusy||voiceListening} style={{padding:'14px 30px',borderRadius:30,border:'none',background:'#7C3AED',color:'#fff',fontSize:17,fontWeight:700,opacity:(voiceBusy||voiceListening)?0.45:1}}>🎤 Falar</button><button onClick={vToggle} style={{padding:'14px 30px',borderRadius:30,border:'1px solid #555',background:'transparent',color:'#fff',fontSize:17}}>✕ Encerrar</button></div><div style={{color:'#777',fontSize:12,marginTop:22}}>Voz em tempo real · PT-BR · gratis</div></div></div>)}{file&&(<div className="fp">{file.preview?<img src={file.preview} className="fp-img" alt="preview"/>:<span>📎 {file.name}</span>}<button onClick={()=>setFile(null)}>✕</button></div>)}
          <div className="ic">
            <button className="ub2" onClick={()=>fileRef.current?.click()} title="Anexar">📎</button>
            <input ref={fileRef} type="file" accept="*/*" onChange={handleFile} style={{display:'none'}}/><button className="ub2" onClick={vToggle} title="Conversar por voz">🎤</button>
          ...
          <div className="in">Daniela V20 🎤📄🔊 · Groq{useQwen?' + Qwen3':''} · Giphy · Browser · Skills · MCP · 24/7</div>
        </div>
      </main>
