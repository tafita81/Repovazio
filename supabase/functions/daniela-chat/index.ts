...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(body&&body.action==='tts'&&body.text){try{const _voice=body.voice||'pt-BR-FranciscaNeural';const _text=String(body.text).substring(0,900).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');const _TR='6A5AA1D4EAFF4E9FB37E23D68491D6F4';const _tk=((BigInt(Math.floor(Date.now()/1000+11644473600))/300n)*300n*10000000n).toString();const _hb=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(_tk+_TR));const _gec=Array.from(new Uint8Array(_hb)).map(b=>b.toString(16).padStart(2,'0')).join('').toUpperCase();const _cid=crypto.randomUUID().replace(/-/g,'');const _url='wss://speech.platform.bing.com/consumer/speech/synthesize/readaloud/edge/v1?TrustedClientToken='+_TR+'&Sec-MS-GEC='+_gec+'&Sec-MS-GEC-Version=1-130.0.2849.68&ConnectionId='+_cid;const _audio=await new Promise((resolve,reject)=>{const _ws=new WebSocket(_url);_ws.binaryType='arraybuffer';const _cks=[];const _to=setTimeout(()=>{try{_ws.close();}catch(e){}reject(new Error('timeout'));},22000);_ws.onopen=()=>{_ws.send('X-Timestamp:'+new Date().toString()+'\r\nContent-Type:application/json; charset=utf-8\r\nPath:speech.config\r\n\r\n{"context":{"synthesis":{"audio":{"metadataoptions":{"sentenceBoundaryEnabled":"false","wordBoundaryEnabled":"false"},"outputFormat":"audio-24khz-48kbitrate-mono-mp3"}}}}');_ws.send('X-RequestId:'+_cid+'\r\nContent-Type:application/ssml+xml\r\nX-Timestamp:'+new Date().toString()+'\r\nPath:ssml\r\n\r\n'+"<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='pt-BR'><voice name='"+_voice+"'><prosody rate='+6%'>"+_text+"</prosody></voice></speak>");};_ws.onmessage=(ev)=>{if(typeof ev.data==='string'){if(ev.data.indexOf('Path:turn.end')!==-1){clearTimeout(_to);try{_ws.close();}catch(e){}let _n=0;for(const c of _cks)_n+=c.length;const _all=new Uint8Array(_n);let _o=0;for(const c of _cks){_all.set(c,_o);_o+=c.length;}resolve(_all);}}else{try{const _dv=new DataView(ev.data);const _hl=_dv.getUint16(0);_cks.push(new Uint8Array(ev.data.slice(2+_hl)));}catch(e){}}};_ws.onerror=()=>{clearTimeout(_to);reject(new Error('ws_error'));};});if(!_audio||_audio.length<100)return new Response(JSON.stringify({error:'tts_failed',detail:'audio_vazio'}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});let _bin='';const _ch=8192;for(let _i=0;_i<_audio.length;_i+=_ch){_bin+=String.fromCharCode.apply(null,Array.from(_audio.subarray(_i,_i+_ch)));}const _b64=btoa(_bin);return new Response(JSON.stringify({audio_base64:_b64,voice:_voice}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}catch(_e){return new Response(JSON.stringify({error:'tts_failed',detail:String(_e&&_e.message||_e)}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}}let modelStr=body.model||'groq:llama-3.3-70b-versatile';if(!body.model){try{const _hr=await fetch(`${SBU}/rest/v1/model_health?status=eq.up&enabled=eq.true&order=rank.asc,latency_ms.asc&limit=1&select=model`,{headers:{apikey:SBK,Authorization:`Bearer ${SBK}`}});if(_hr.ok){const _ha=await _hr.json();if(_ha[0]&&_ha[0].model)modelStr=_ha[0].model;}}catch(_e){}}
