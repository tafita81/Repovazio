...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(body&&body.action==='tts'&&body.text){try{const _voice=body.voice||'Aoede';const _gk=await fetch(SBU+'/rest/v1/ia_cache?cache_key=eq.secret:GEMINI_API_KEY_NEW&select=value',{headers:{apikey:SBK,Authorization:'Bearer '+SBK}});const _gka=await _gk.json();const _gkey=_gka&&_gka[0]&&_gka[0].value;const _gr=await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-tts:generateContent?key='+_gkey,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:String(body.text).substring(0,1200)}]}],generationConfig:{responseModalities:['AUDIO'],speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName:_voice}}}}})});if(!_gr.ok){const _et=await _gr.text();return new Response(JSON.stringify({error:'tts_failed',detail:'gm'+_gr.status+'_'+_et.substring(0,80)}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}const _gj=await _gr.json();const _pcmb64=_gj&&_gj.candidates&&_gj.candidates[0]&&_gj.candidates[0].content&&_gj.candidates[0].content.parts&&_gj.candidates[0].content.parts[0]&&_gj.candidates[0].content.parts[0].inlineData&&_gj.candidates[0].content.parts[0].inlineData.data;if(!_pcmb64)return new Response(JSON.stringify({error:'tts_failed',detail:'sem_audio'}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});const _pcmU8=Uint8Array.from(atob(_pcmb64),c=>c.charCodeAt(0));const _pcmLen=_pcmU8.length;const _wav=new Uint8Array(44+_pcmLen);const _dv=new DataView(_wav.buffer);const _ws=(o,s)=>{for(let i=0;i<s.length;i++)_dv.setUint8(o+i,s.charCodeAt(i));};const _sr=24000;_ws(0,'RIFF');_dv.setUint32(4,36+_pcmLen,true);_ws(8,'WAVE');_ws(12,'fmt ');_dv.setUint32(16,16,true);_dv.setUint16(20,1,true);_dv.setUint16(22,1,true);_dv.setUint32(24,_sr,true);_dv.setUint32(28,_sr*2,true);_dv.setUint16(32,2,true);_dv.setUint16(34,16,true);_ws(36,'data');_dv.setUint32(40,_pcmLen,true);_wav.set(_pcmU8,44);let _bin='';const _ch=8192;for(let _i=0;_i<_wav.length;_i+=_ch){_bin+=String.fromCharCode.apply(null,Array.from(_wav.subarray(_i,_i+_ch)));}const _b64=btoa(_bin);return new Response(JSON.stringify({audio_base64:_b64,format:'wav',voice:_voice}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}catch(_e){return new Response(JSON.stringify({error:'tts_failed',detail:String(_e&&_e.message||_e)}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}}
if(body&&body.action==='arena'&&body.text){try{const _q=String(body.text).substring(0,2000);const _sys='Voce e Daniela Coelho, pesquisadora de comportamento humano. Responda em PT-BR, util e claro. NUNCA use a palavra psicologa nem psicologo. Seja conciso, ate cerca de 150 palavras.';const _kr=await fetch(SBU+'/rest/v1/ia_cache?cache_key=in.(secret:GROQ_API_KEY,secret:NVIDIA_API_KEY,secret:GEMINI_API_KEY_NEW)&select=cache_key,value',{headers:{apikey:SBK,Authorization:'Bearer '+SBK}});const _ka=await _kr.json();const _km={};for(const _row of _ka){_km[_row.cache_key]=_row.value;}const _gq=_km['secret:GROQ_API_KEY'];const _nv=_km['secret:NVIDIA_API_KEY'];const _gm=_km['secret:GEMINI_API_KEY_NEW'];const _contest=[{name:'Groq Llama-3.3 70B',kind:'oai',url:'https://api.groq.com/openai/v1/chat/completions',key:_gq,model:'llama-3.3-70b-versatile'},{name:'NVIDIA DeepSeek V4',kind:'oai',url:'https://integrate.api.nvidia.com/v1/chat/completions',key:_nv,model:'deepseek-ai/deepseek-v4-flash'},{name:'NVIDIA Llama-4 Maverick',kind:'oai',url:'https://integrate.api.nvidia.com/v1/chat/completions',key:_nv,model:'meta/llama-4-maverick-17b-128e-instruct'},{name:'Google Gemini 2.5 Flash',kind:'gemini',url:'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key='+_gm,model:'gemini-2.5-flash'},{name:'Groq Llama-4 Scout',kind:'oai',url:'https://api.groq.com/openai/v1/chat/completions',key:_gq,model:'meta-llama/llama-4-scout-17b-16e-instruct'}];async function _callOne(c){const _t0=Date.now();try{if(c.kind==='gemini'){const r=await fetch(c.url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({systemInstruction:{parts:[{text:_sys}]},contents:[{parts:[{text:_q}]}],generationConfig:{maxOutputTokens:500}}),signal:AbortSignal.timeout(15000)});const j=await r.json();const t=j&&j.candidates&&j.candidates[0]&&j.candidates[0].content&&j.candidates[0].content.parts&&j.candidates[0].content.parts[0]&&j.candidates[0].content.parts[0].text;if(!r.ok||!t)throw new Error('http'+r.status);return{model:c.name,reply:t,ms:Date.now()-_t0,ok:true};}else{const r=await fetch(c.url,{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+c.key},body:JSON.stringify({model:c.model,messages:[{role:'system',content:_sys},{role:'user',content:_q}],max_tokens:500,temperature:0.7}),signal:AbortSignal.timeout(15000)});const j=await r.json();const t=j&&j.choices&&j.choices[0]&&j.choices[0].message&&j.choices[0].message.content;if(!r.ok||!t)throw new Error('http'+r.status);return{model:c.name,reply:t,ms:Date.now()-_t0,ok:true};}}catch(e){return{model:c.name,reply:'(indisponivel: '+String(e&&e.message||e)+')',ms:Date.now()-_t0,ok:false};}}const _res=await Promise.all(_contest.map(_callOne));let _best=-1,_why='';const _ok=_res.filter(function(x){return x.ok;});if(_ok.length===1){_best=_res.indexOf(_ok[0]);_why='unica resposta valida';}else if(_ok.length>1){const _jp='Pergunta: '+_q+'\n\nRespostas dos modelos:\n'+_res.map(function(x,i){return '['+i+'] '+x.model+': '+String(x.reply).substring(0,700);}).join('\n\n')+'\n\nQual indice (numero) tem a melhor resposta (mais precisa, util e clara)? Responda SO em JSON: {"best":N,"why":"motivo curto em pt-BR"}';try{const jr=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+_gq},body:JSON.stringify({model:'llama-3.3-70b-versatile',messages:[{role:'user',content:_jp}],max_tokens:150,temperature:0,response_format:{type:'json_object'}}),signal:AbortSignal.timeout(15000)});const jj=await jr.json();const jt=jj&&jj.choices&&jj.choices[0]&&jj.choices[0].message&&jj.choices[0].message.content;if(jr.ok&&jt){const pj=JSON.parse(jt);_best=(typeof pj.best==='number')?pj.best:-1;_why=pj.why||'';}}catch(e){}if(_best<0){try{const gr=await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key='+_gm,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:_jp}]}],generationConfig:{responseMimeType:'application/json',maxOutputTokens:150}}),signal:AbortSignal.timeout(15000)});const gj=await gr.json();const gt=gj&&gj.candidates&&gj.candidates[0]&&gj.candidates[0].content&&gj.candidates[0].content.parts&&gj.candidates[0].content.parts[0]&&gj.candidates[0].content.parts[0].text;if(gt){const pj=JSON.parse(gt);_best=(typeof pj.best==='number')?pj.best:-1;_why=(pj.why||'')+' [juiz Gemini]';}}catch(e){}}}return new Response(JSON.stringify({results:_res,best:_best,why:_why}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}catch(_ae){return new Response(JSON.stringify({error:'arena_failed',detail:String(_ae&&_ae.message||_ae)}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}}let modelStr=body.model||'groq:llama-3.3-70b-versatile';if(!body.model){try{const _hr=await fetch(`${SBU}/rest/v1/model_health?status=eq.up&enabled=eq.true&order=rank.asc,latency_ms.asc&limit=1&select=model`,{headers:{apikey:SBK,Authorization:`Bearer ${SBK}`}});if(_hr.ok){const _ha=await _hr.json();if(_ha[0]&&_ha[0].model)modelStr=_ha[0].model;}}catch(_e){}}
    const userMsgs=Array.isArray(body.messages)?body.messages:[];const _att=body.image;if(_att&&typeof _att==='string'&&userMsgs.length){const _i=userMsgs.length-1;const _l=userMsgs[_i];if(_l&&_l.role==='user'){const _t=typeof _l.content==='string'?_l.content:'';if(_att.startsWith('data:image/')){userMsgs[_i]={role:'user',content:[{type:'text',text:_t||'Descreva e analise esta imagem em detalhe.'},{type:'image_url',image_url:{url:_att}}]};modelStr='groq:meta-llama/llama-4-scout-17b-16e-instruct';}else if(_att.startsWith('data:application/pdf')){let _ptxt='';try{const _b64=_att.split(',')[1]||'';const _bin=atob(_b64);const _arr=new Uint8Array(_bin.length);for(let _k=0;_k<_bin.length;_k++)_arr[_k]=_bin.charCodeAt(_k);const _unp=await import('https://esm.sh/unpdf@0.12.1');const _doc=await _unp.getDocumentProxy(_arr);const _ex=await _unp.extractText(_doc,{mergePages:true});_ptxt=typeof _ex.text==='string'?_ex.text:(Array.isArray(_ex.text)?_ex.text.join(' '):'');}catch(_e){_ptxt='(falha ao extrair texto do PDF)';}userMsgs[_i]={..._l,content:'O usuario anexou um documento PDF; o texto extraido esta abaixo. Faca uma analise PROFUNDA, ESTRUTURADA e UTIL: contextualize, destaque os pontos-chave, cite numeros e dados concretos, aponte implicacoes e proximos passos quando fizer sentido. Portugues brasileiro claro. NUNCA diga que nao conseguiu ler nem peca mais informacoes; o conteudo esta abaixo.\n\n[CONTEUDO DO PDF]:\n'+_ptxt.substring(0,24000)+'\n\n[PEDIDO DO USUARIO]: '+(_t||'Faca uma analise completa, estruturada e detalhada deste documento.')};modelStr='groq:llama-3.3-70b-versatile';}else{userMsgs[_i]={..._l,content:'O usuario anexou um arquivo; o conteudo esta abaixo. Analise com profundidade, de forma estruturada e util, citando dados concretos. Portugues brasileiro. NUNCA diga que nao conseguiu ler nem peca mais informacoes.\n\n[CONTEUDO DO ARQUIVO ANEXADO]:\n'+_att.substring(0,24000)+'\n\n[PEDIDO DO USUARIO]: '+(_t||'Faca uma analise completa e estruturada deste arquivo.')};modelStr='groq:llama-3.3-70b-versatile';}}}
    const modelDisplay=
