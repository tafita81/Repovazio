...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(body&&body.action==='tts'&&body.text){try{const _voice=body.voice||'Camila';const _text=encodeURIComponent(String(body.text).substring(0,550));const _tr=await fetch('https://api.streamelements.com/kappa/v2/speech?voice='+_voice+'&text='+_text,{headers:{'User-Agent':'Mozilla/5.0','Accept':'audio/mpeg'}});if(!_tr.ok){const _et=await _tr.text();return new Response(JSON.stringify({error:'tts_failed',detail:'se'+_tr.status+'_'+_et.substring(0,80)}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}const _ab=await _tr.arrayBuffer();const _u8=new Uint8Array(_ab);if(_u8.length<100)return new Response(JSON.stringify({error:'tts_failed',detail:'audio_vazio'}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});let _bin='';const _ch=8192;for(let _i=0;_i<_u8.length;_i+=_ch){_bin+=String.fromCharCode.apply(null,Array.from(_u8.subarray(_i,_i+_ch)));}const _b64=btoa(_bin);return new Response(JSON.stringify({audio_base64:_b64,voice:_voice}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}catch(_e){return new Response(JSON.stringify({error:'tts_failed',detail:String(_e&&_e.message||_e)}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}}let modelStr=body.model||'groq:llama-3.3-70b-versatile';if(!body.model){try{const _hr=await fetch(`${SBU}/rest/v1/model_health?status=eq.up&enabled=eq.true&order=rank.asc,latency_ms.asc&limit=1&select=model`,{headers:{apikey:SBK,Authorization:`Bearer ${SBK}`}});if(_hr.ok){const _ha=await _hr.json();if(_ha[0]&&_ha[0].model)modelStr=_ha[0].model;}}catch(_e){}}
