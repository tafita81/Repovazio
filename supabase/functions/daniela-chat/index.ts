...
const TOOLS=[
  ...
  {type:'function',function:{name:'browser_screenshot',description:'Screenshot de URL',parameters:{type:'object',properties:{url:{type:'string'}},required:['url']}}},{type:'function',function:{name:'browser_task',description:'Agente web REAL (Playwright): abre pagina, DIGITA em campos, CLICA botoes, extrai dados e tira screenshot, sozinho. steps[] com acoes: goto{url}, type{selector,text}, click{selector}, press{key}, wait_for{selector}, extract{selector,name}, extract_all{selector,name,limit}, screenshot. Retorna task_id; resultado fica em browser_tasks (status done em ~2-3min).',parameters:{type:'object',properties:{goal:{type:'string'},steps:{type:'array',items:{type:'object'}}},required:['goal','steps']}}},
