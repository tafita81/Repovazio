...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(body&&body.action==='tts'&&body.text){try{const _kr=await fetch(SBU+'/rest/v1/ia_cache?cache_key=eq.secret:ELEVENLABS_API_KEY&select=value',{headers:{apikey:SBK,Authorization:'Bearer '+SBK}});const _ka=await _kr.json();const _ek=_ka&&_ka[0]&&_ka[0].value;if(!_ek)return new Response(JSON.stringify({error:'no_key'}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});const _vid=body.voice_id||'EXAVITQu4vr4xnSDxMaL';const _tr=await fetch('https://api.elevenlabs.io/v1/text-to-speech/'+_vid+'?output_format=mp3_44100_128',{method:'POST',headers:{'xi-api-key':_ek,'Content-Type':'application/json'},body:JSON.stringify({text:String(body.text).substring(0,700),model_id:'eleven_multilingual_v2',voice_settings:{stability:0.45,similarity_boost:0.85,style:0.15,use_speaker_boost:true}})});if(!_tr.ok){const _et=await _tr.text();return new Response(JSON.stringify({error:'tts_failed',detail:_et.substring(0,160)}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}const _ab=await _tr.arrayBuffer();const _u8=new Uint8Array(_ab);let _bin='';const _ch=8192;for(let _i=0;_i<_u8.length;_i+=_ch){_bin+=String.fromCharCode.apply(null,Array.from(_u8.subarray(_i,_i+_ch)));}const _b64=btoa(_bin);return new Response(JSON.stringify({audio_base64:_b64}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}catch(_e){return new Response(JSON.stringify({error:String(_e&&_e.message||_e)}),{status:200,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});}}let modelStr=body.model||'groq:llama-3.3-70b-versatile';if(!body.model){try{const _hr=await fetch(`${SBU}/rest/v1/model_health?status=eq.up&enabled=eq.true&order=rank.asc,latency_ms.asc&limit=1&select=model`,{headers:{apikey:SBK,Authorization:`Bearer ${SBK}`}});if(_hr.ok){const _ha=await _hr.json();if(_ha[0]&&_ha[0].model)modelStr=_ha[0].model;}}catch(_e){}}
